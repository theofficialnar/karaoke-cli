...
public class SongBook {
  ...
  private Map<String, List<Song>> byArtist() {
    ...
    // Loop through all the songs in the song book
    for(Song song: mSongs) {
      // Check if artist name exists in song book
      List<Song> artistSongs = byArtist.get(song.getArtist());
      // If artist name doesn't exist, initialize it and add it to the song book
      if(artistSongs == null) {
        artistSongs = new ArrayList<Song>();
        byArtist.put(song.getArtist(), artistSongs);
      }
      // If artist name already exists, just add the song to his name
